...
class VentuzStreamPlayer extends HTMLElement {
    ...
    private openStream() {
        ...
        if (!this.video || !this.streamHeader)
            return;

        this.mediaSource = new MediaSource();
        this.mediaSource.onsourceopen = () => {
            console.log("source open");
            this.vidSrcBuffer = this.mediaSource!.addSourceBuffer('video/mp4; codecs="avc1.640C20"');
        ...
        this.muxer = new Mp4Muxer.Muxer({
            ...
            video:
            {
                ...
                frameRate: this.streamHeader.videoFrameRateNum / this.streamHeader.videoFrameRateDen,
            },
    ...
    private handlePacket(pkg: StreamOut.StreamPacket) {
        switch (pkg.type) {
            ...
            case "frame":
                ...
                break;
            default: throw new Error("pkg syntax");
        }
    }
    ...
    private handleVideoFrame(data: Uint8Array) {
        if (this.muxer && this.streamHeader && this.frameHeader) {
            ...
            const dur = 1000000 * this.streamHeader.videoFrameRateDen / this.streamHeader.videoFrameRateNum;
            ...
            // const meta = null;
            this.muxer.addVideoChunkRaw(data, key ? "key" : "delta", ts, dur, meta);
    ...
    connectedCallback() {
        ...
        this.video.onpointerdown = e => {
            ...
            if (e.pointerType === "touch") {
                this.sendCommand({ type: "touchBegin", data: { ...getIntXY(e.clientX, e.clientY), id: e.pointerId } });
            }
        ...
        this.video.onpointerup = e => {
            ...
            if (e.pointerType === "touch") {
                this.sendCommand({ type: "touchEnd", data: { ...getIntXY(e.clientX, e.clientY), id: e.pointerId } });
            }
        ...
        this.video.onpointermove = e => {
            ...
            if (e.pointerType === "mouse") {
                this.sendCommand({ type: "mouseMove", data: getIntXY(e.x, e.y) })
            }

            if (e.pointerType === "touch") {
                this.sendCommand({ type: "touchMove", data: { ...getIntXY(e.clientX, e.clientY), id: e.pointerId } });
            }
        ...
        this.video.onpointercancel = e => {
            ...
            if (e.pointerType === "touch") {
                this.sendCommand({ type: "touchCancel", data: { ...getIntXY(e.clientX, e.clientY), id: e.pointerId } });
            }
        ...
        this.video.onwheel = e => {
            this.sendCommand({ type: "mouseWheel", data: { x: -e.deltaX, y: -e.deltaY, } });
            e.stopPropagation();
            e.preventDefault();
        }
        ...
        this.video.onkeydown = e => {
            ...
            this.sendCommand({ type: "char", data: e.keyCode >= 32 ? e.key.charCodeAt(0) : e.keyCode });
            e.stopPropagation();
            e.preventDefault();
        }
