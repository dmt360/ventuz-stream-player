...
export class MP4Remuxer {
    ...
    pushVideo(sn: number, videoTrack: MP4.VideoTrack) {
        this.sn = sn
    ...
    remuxVideo_2(track: MP4.VideoTrack) {
        var offset = 8,
            mdat,
            moof,
    ...
    generateVideoIS(videoTrack: MP4.VideoTrack) {
    ...
    remux(
        ...
        id3Track: MP4.DataTrack,textTrack: MP4.DataTrack,
        timeOffset: number,
        contiguous: boolean
    ) {
        this.sn = sn
    ...
    generateIS(
        audioTrack: MP4.AudioTrack, videoTrack: MP4.VideoTrack,
        timeOffset: number
    ) {
        var //audioSamples = audioTrack.samples,
            videoSamples = videoTrack.samples,
            pesTimeScale = this.PES_TIMESCALE,
            tracks: { video?: any; audio?: any } = {},
    ...
    if (audioTrack.config && audioSamples.length) {
      ...
      if (audioTrack.timescale * audioTrack.duration > Math.pow(2, 32)) {
        let greatestCommonDivisor = function(a: number, b: number) {
            if ( ! b) {
                return a;
            }
    ...
    remuxVideo(
        ...
        for (let i = 0; i < inputSamples.length; i++) {
            ...
            sample.pts =
                Math.round(sample.pts / pes2mp4ScaleFactor) * pes2mp4ScaleFactor
        }
  ...
  remuxAudio(track: MP4.AudioTrack, timeOffset: number, contiguous: boolean) {
